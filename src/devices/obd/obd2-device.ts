...
export class Obd2Device extends AbstractSerialDevice {
  ...
  private static INIT_COMMANDS: string[] = ['ATZ\r\n', 'ATSP0\r\n', '0100\r\n'];
  ...
  onData(inData: string): any {
    debugger;
    const data: string = inData.trim().replace(/[\r]+/g, '');
  ...
  private processSpeedResult(data: string): void {
    ...
    if (data == 'NO DATA') {
      ...
      const kph: number = this.extractResultAsNumber(data);
  ...
  private extractResultAsNumber(data: string): number {
    ...
    try {
      const toParse = data.substring(data.length - 2);
